...
StackType_t *pxPortInitialiseStack( StackType_t *pxTopOfStack, TaskFunction_t pxCode, void *pvParameters )/*初始化任务栈*/
{
 ...
 */
__asm void prvStartFirstTask( void )/*主要工作是寻找msp的地址*/
{
	PRESERVE8
...
void vPortSetupTimerInterrupt( void )/*初始化Systick函数*/
{
     /* 设置重装载寄存器的值 */
...
void xPortSysTickHandler( void )/*Systick中断服务函数*/
{
	/* 关中断 */
    vPortRaiseBASEPRI();
